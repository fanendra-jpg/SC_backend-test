...
exports.getSchoolsuser = async (req, res) => {
  try {
    const { price, adminId } = req.query;

    let schools = await School.find({}, { updatedBy: 0 }).lean();

    const schoolIds = schools.map((s) => s._id);

    const activeAdmins = await Admin.find({ status: true }).select("_id").lean();
    let adminIds = activeAdmins.map((a) => a._id);
    ...
    const prices = await InstitutionPrice.find({
      classId: { $in: schoolIds },
      type: "school",
    ...
    const priceMap = {};
    prices.forEach((p) => {
      priceMap[p.classId.toString()] = p;
    });

    for (let school of schools) {
      const priceData = priceMap[school._id.toString()];

      if (priceData) {
        school.price = priceData.price;
        school.updatedBy = priceData.updatedBy;
      } else {
        school.price = null;
    ...
    if (price) {
      schools = schools.filter((s) => s.price !== null);
    }
...
exports.getSchools = async (req, res) => {
  try {
    ...
    for (let school of schools) {
      ...
      if (priceData) {
        school.price = priceData.price;
        school.updatedBy = priceData.updatedBy || priceData.createdBy;
      } else {
        school.price = null;
        school.updatedBy = null;
      }
...
exports.getCollege = async (req, res) => {
  try {
    ...
    let colleges = await College.find({}, { updatedBy: 0 }).lean();
    const collegeIds = colleges.map((c) => c._id);

    const prices = await InstitutionPrice.find({
      classId: { $in: collegeIds },
      type: "college",
      updatedBy: adminId   
    }).lean();
    ...
    for (let college of colleges) {
      const priceData = priceMap[college._id.toString()];

      if (priceData) {
        college.price = priceData.price;
        college.updatedBy = priceData.updatedBy || priceData.createdBy;
      } else {
        college.price = null;
        college.updatedBy = null;
      }
    }

    if (price) {
      colleges = colleges.filter((c) => c.price !== null);
    }
...
//       if (priceData) {
//         college.price = priceData.price;
//         college.updatedBy = priceData.updatedBy;
//       } else {
//         college.price = null;
//         college.updatedBy = null;
//       }
...
exports.updateInstitution = async (req, res) => {
  try {
    ...
    if (!["college", "school"].includes(type.toLowerCase())) {
      return res
        .status(400)
        .json({ message: 'Invalid type. Type must be either "college" or "school".' });
    }
    ...
    // price update in InstitutionPrice collection
    if (price !== undefined) {
      ...
      await InstitutionPrice.findOneAndUpdate(
        {
          classId: id,
...
exports.Userinstitute = async (req, res) => {
  try {
    const { price, adminId } = req.query;

    const schools = await School.find({}, { updatedBy: 0 }).lean();
    const colleges = await College.find({}, { updatedBy: 0 }).lean();
    ...
    const prices = await InstitutionPrice.find({
      updatedBy: { $in: adminIds },
      $or: [
        { classId: { $in: schoolIds }, type: "school" },
        { classId: { $in: collegeIds }, type: "college" }
      ]
    }).lean();
    ...
    let institutes = [...schools, ...colleges];

    if (price) {
      institutes = institutes.filter((i) => i.price !== null);
    }
...
exports.institute = async (req, res) => {
  try {
    ...
    const prices = await InstitutionPrice.find({
      updatedBy: adminId, // 🔑 admin-wise price
      $or: [
        { classId: { $in: schoolIds }, type: "school" },
        { classId: { $in: collegeIds }, type: "college" }
      ]
  ...
  try {
    ...
    if (!["college", "school"].includes(type.toLowerCase())) {
      return res.status(400).json({
        message: 'Invalid type. Must be either "college" or "school".'
      });
    }

    const priceData = await InstitutionPrice.findOneAndUpdate(
      { 
        classId: id, 
        type: type.toLowerCase(),
        updatedBy: updatedBy
...
exports.deleteSchoolPrice = async (req, res) => {
  try {
    ...
    const deletedPrice = await InstitutionPrice.findOneAndDelete({
      classId: id,
      type: "school",
      updatedBy: adminId   // 🔑 sirf same admin ka price delete hoga
    });

    if (!deletedPrice) {
      return res.status(404).json({
        message: "Price not found for this school for this admin."
      });
    }

    res.status(200).json({
      message: "Price removed from school successfully.",
      data: deletedPrice,
    });
...
exports.deleteCollegePrice = async (req, res) => {
  try {
    ...
    if (!deletedPrice) {
      return res.status(404).json({
        message: "Price not found for this college for this admin."
      });
    }

    res.status(200).json({
      message: "Price removed from college successfully.",
      data: deletedPrice
    });
