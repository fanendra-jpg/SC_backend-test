...
//     // ✅ STEP 1: SESSION CARD SE END DATE LO
//     const sessionData = await SessionCard.findOne({ userId })
//       .sort({ createdAt: -1 }) // latest session
//       .lean();
...
//       finalEndDate = sessionData.endDate;
//       finalSession = sessionData.session;
//     } else {
//       // ✅ STEP 2: FALLBACK USER TABLE
//       finalEndDate = user.endDate;
//       finalSession = user.session;
//     }

//     if (!finalEndDate || !user.className) {
//       return res.status(400).json({
//         message: "EndDate or Class not found"
//       });
//     }
...
//       : moment(user.updatedAt || user.createdAt).startOf("day");
...
//     // ❌ NOTE: history save same hi rakha (optional)
...
exports.scoreCard = async (req, res) => {
  try {
    ...
    const isHistoryUser = !!user.originalUserId;
...
//     // ✅ BASE MATCH (🔥 dynamic user / history dono ke liye same)
//     const baseMatch = {
//       userId: new mongoose.Types.ObjectId(userId),
//       endDate: user.endDate,
//       classId: user.className.toString()
...
//     // ✅ SAVE ONLY IF MAIN USER (history me save nahi karna)
//     if (!queryEndDate) {
//       for (const item of learningWiseAverage) {
//         const idx = user.learning.findIndex(
//           l =>
...
exports.Dashboard = async (req, res) => {
  try {
    ...
    if (learningScores.length && topicScores.length) {
      const practiceDates = new Set(
        learningScores.map(s =>
          moment(s.scoreDate).format('YYYY-MM-DD')
        )
      );

      const topicDates = new Set(
        topicScores.map(s =>
          moment(s.updatedAt).format('YYYY-MM-DD')
        )
      );
      ...
      for (const item of assignedLearnings) {
        ...
        const getIQ = async (field) => {
          ...
          const iq = await GenralIQ.findOne({
            userId,
            endDate: user.endDate, // ✅ auto handled
            classId,
    ...
    if (expData?.levelBonusPoint != null) {
      ...
      const ld = user.userLevelData.find(
